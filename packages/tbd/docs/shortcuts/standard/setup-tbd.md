...
3. github-merge (recommended: per-request): <meaning>
...
| A policy’s not-granted value | `tbd policy revoke <policy>` |
...
Record `github-merge: unconditional` or `pr-review-requirements: none` only when the

...
describe('tbd-format', () => {
  ...
  describe('generated integration format (split from the repository format)', () => {
    // What every pre-split release compares a stamp against: through tbd 0.9.0,
    // AGENT_INTEGRATION_FORMAT aliased CURRENT_FORMAT, which was f08.
    const PRE_SPLIT_CEILING = 'f08';
    const agentsMdBeginLine = `<!-- BEGIN TBD INTEGRATION format=${AGENT_INTEGRATION_FORMAT} surface=agents-md -->`;
    const skillMarker = `<!-- DO NOT EDIT: Generated by tbd setup (format=${AGENT_INTEGRATION_FORMAT}).\nRun 'tbd setup' to update.\n-->`;

    function stampNumber(content: string): number {
      const found = parseManagedIntegrationFormat(content);
      if (found === null) {
        throw new Error(`expected a format= stamp in: ${content}`);
      }
      return integrationFormatNumber(found);
    }

    it('starts the integration series at f100 while the repository format stays f08', () => {
      expect(AGENT_INTEGRATION_FORMAT).toBe('f100');
      expect(CURRENT_FORMAT).toBe('f08');
    });

    it('can never be read as a repository format', () => {
      // Three digits keep the series apart from the two-digit repository formats, and
      // FORMAT_HISTORY membership (not numeric order) decides repository compatibility,
      // so no code path can accept the integration stamp as a tbd_format value.
      expect(AGENT_INTEGRATION_FORMAT).toMatch(/^f\d{3}$/);
    ...
    it('makes tbd 0.9.0 and older refuse to rewrite the stamped surfaces', () => {
      // parseManagedIntegrationFormat and integrationFormatNumber are byte-identical to
      // the v0.9.0 tag, and 0.9.0's setup and doctor guards are exactly this comparison
      // against its ceiling of f08. The refusal is what keeps a policy grants block
      // inside the AGENTS.md block from being deleted by an older release.
      for (const surface of [agentsMdBeginLine, skillMarker]) {
        expect(parseManagedIntegrationFormat(surface)).toBe(AGENT_INTEGRATION_FORMAT);
        expect(stampNumber(surface)).toBeGreaterThan(integrationFormatNumber(PRE_SPLIT_CEILING));
      }
    });

    it('outranks every possible pre-split ceiling, not only f08', () => {
      // A pre-split release aliases the repository format, which is at most two digits.
      for (let n = 1; n <= 99; n++) {
        const ceiling = `f${String(n).padStart(2, '0')}`;
        expect(integrationFormatNumber(AGENT_INTEGRATION_FORMAT)).toBeGreaterThan(
          integrationFormatNumber(ceiling),
        );
      }
    });

    it('lets this release upgrade pre-split stamps in place', () => {
      // A block written by 0.9.0 (format=f08) or by a release before stamping (no
      // format= field, read as f01) is older than the integration format, so setup
      // rewrites it instead of refusing.
      expect(
        stampNumber('<!-- BEGIN TBD INTEGRATION format=f08 surface=agents-md -->'),
      ).toBeLessThan(integrationFormatNumber(AGENT_INTEGRATION_FORMAT));
      expect(parseManagedIntegrationFormat('<!-- BEGIN TBD INTEGRATION -->')).toBeNull();
      expect(integrationFormatNumber('f01')).toBeLessThan(
        integrationFormatNumber(AGENT_INTEGRATION_FORMAT),
      );
    });
  });

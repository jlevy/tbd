...
describe('setup flows', { timeout: subprocessTestTimeout() }, () => {
  ...
  describe('fresh repo setup', () => {
    ...
    it('refuses to overwrite a generated skill written by a newer tbd', async () => {
      ...
      await writeFile(
        portablePath,
        '---\nname: tbd\n---\n' +
          "<!-- DO NOT EDIT: Generated by tbd setup (format=f999).\nRun 'tbd setup' to update.\n-->\n\nFuture skill.\n",
      );
      ...
      expect(preserved).toContain('format=f999');
  ...
  describe('AGENTS.md compact managed block', () => {
    it('writes a compact, format-stamped block instead of the full skill', async () => {
      ...
      // The block carries the generated integration format, not the repository format.
      expect(agents).toContain(
        `<!-- BEGIN TBD INTEGRATION format=${AGENT_INTEGRATION_FORMAT} surface=agents-md -->`,
      );
  ...
  describe('integration format guard (self-upgrade safety)', () => {
    it('self-upgrades a legacy unversioned AGENTS.md block in place', async () => {
      ...
      expect(agents).toContain(`format=${AGENT_INTEGRATION_FORMAT}`);
      // ...while preserving user content outside the managed region.
      expect(agents).toContain('## My Notes');
      expect(agents).toContain('Keep me.');
    });

    it('upgrades an AGENTS.md block stamped by the last pre-split release in place', async () => {
      initGitRepo();
      // tbd 0.9.0 stamped the repository format (f08) into the block. That is below the
      // integration format, so this release rewrites the block rather than refusing.
      await writeFile(
        join(tempDir, 'AGENTS.md'),
        '# Project Instructions for AI Agents\n\n' +
          `<!-- BEGIN TBD INTEGRATION format=${CURRENT_FORMAT} surface=agents-md -->\n` +
          '## tbd\n\nBlock written by tbd 0.9.0.\n' +
          '<!-- END TBD INTEGRATION -->\n\n## My Notes\n\nKeep me.\n',
      );
      ...
      expect(agents).toContain(`format=${AGENT_INTEGRATION_FORMAT}`);
      expect(agents).not.toContain(`format=${CURRENT_FORMAT} `);
      expect(agents).not.toContain('Block written by tbd 0.9.0.');
      expect(agents).toContain('## My Notes');
      expect(agents).toContain('Keep me.');
    });
    ...
    it('refuses to overwrite an AGENTS.md written by a newer tbd', async () => {
      ...
      await writeFile(
        join(tempDir, 'AGENTS.md'),
        '# Project Instructions for AI Agents\n\n' +
          '<!-- BEGIN TBD INTEGRATION format=f999 surface=agents-md -->\n' +
          '## tbd\n\nFuture block.\n' +
          '<!-- END TBD INTEGRATION -->\n',
      );
      ...
      const agents = await readFile(join(tempDir, 'AGENTS.md'), 'utf-8');
      expect(agents).toContain('format=f999');

...
## Uninstall Removes Generated Tier Agent Definitions

Setup generates the `tbd-*` tier agent definitions as managed files; uninstall removes
the generated ones and keeps a user-owned file under a tbd name.

# Test: Reinitialize with the tier agent surfaces only

```console
$ tbd setup --auto --prefix=test --surfaces=claude-agents,codex-agents > /dev/null
? 0
```

# Test: Eight definitions are generated

```console
$ ls .claude/agents .codex/agents
.claude/agents:
tbd-fast.md
tbd-moderate.md
tbd-strong-max.md
...
# Test: Replace one with a user-owned definition (no tbd marker)
...
# Test: Uninstall preview counts only the generated files

```console
$ tbd uninstall | grep "Tier agent"
  - Tier agent definitions: 7 files (.claude/agents/tbd-*.md, .codex/agents/tbd-*.toml)
? 0
```

# Test: Uninstall removes the generated definitions

```console
$ tbd uninstall --confirm | grep "tier agent"
  ✓ Removed 7 tier agent definitions
? 0
```

# Test: The user-owned file is the only one left
...
# Test: The emptied Codex agents directory is gone

```console
$ test -d .codex/agents || echo "no .codex/agents"
no .codex/agents
? 0
```

# Test: Removing the user-owned file leaves a clean repository

```console
$ rm -r .claude && git status --porcelain
? 0
```

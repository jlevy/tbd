...
/**
 * Read the integration format stamped in a generated artifact. Returns the
 * stamped number, 1 for a legacy marked block with no metadata comment, or null
 * when there is no tbd-managed block at all.
 */
function parseIntegrationFormat(content: string): number | null {
  const match = /tbd:integration-format=(\d+)/.exec(content);
  if (match?.[1]) {
    return Number.parseInt(match[1], 10);
  }
  return content.includes(CODEX_BEGIN_MARKER) ? 1 : null;
}
...
function assertNotNewerFormat(content: string, artifact: string): void {
  ...
  if (format !== null && format > AGENT_INTEGRATION_FORMAT) {
    throw new CLIError(

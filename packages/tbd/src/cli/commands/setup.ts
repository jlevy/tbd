...
/**
 * Every file the Claude Code surface writes, in one list so the target check and
 * the writers cannot drift apart.
 */
function claudeSurfaceTargets(cwd: string): string[] {
  const paths = getClaudePaths(cwd);
  return [
    paths.settings,
    paths.sessionScript,
    paths.ghCliScript,
    paths.closingReminder,
    join(paths.dir, '.gitignore'),
    paths.skill,
  ];
}
...
export interface InspectTierAgentSurfaceOptions {
  /** Verb for a refusal message; `tbd uninstall` and `tbd doctor` do not update. */
  operation?: string;
  /**
   * Called instead of throwing when a path is refused. `tbd uninstall` uses this
   * to treat an unsafe path as not tbd's, warn, and finish removing the rest;
   * setup and doctor let the refusal propagate so the surface reports it.
   */
  onRefused?: (refusal: ManagedArtifactTargetError) => void;
}
...
export async function inspectTierAgentSurface(
  cwd: string,
  platform: TierAgentPlatform,
  options: InspectTierAgentSurfaceOptions = {},
  ...
  for (const definition of TIER_AGENT_DEFINITIONS) {
    ...
    try {
      await assertSafeManagedArtifactTarget(path, {
        projectRoot: cwd,
        allowMissing: true,
        operation: options.operation ?? 'update',
      });
    } catch (error) {
      if (options.onRefused && error instanceof ManagedArtifactTargetError) {
 ...
 */
export async function listGeneratedTierAgentFiles(
  cwd: string,
  options: InspectTierAgentSurfaceOptions = {},
): Promise<TierAgentFileState[]> {
  const generated: TierAgentFileState[] = [];
  for (const platform of ['claude', 'codex'] as const) {
    for (const file of await inspectTierAgentSurface(cwd, platform, options)) {
...
class SetupCodexHandler extends BaseCommand {
  ...
  private async withAgentsMdLock(
    ...
    if (this.ctx.dryRun) {
      // A dry run computes and prints; there is nothing to serialize, and it
      // must not create the lock directory.
      await operation();
      return;
    }
    await withAgentsMdLock(dirname(agentsPath), operation);
  }
  ...
  private async handleAlreadyInitialized(
    ...
    const tbdGitignoreResult = await ensureGitignorePatterns(
      ...
      [
        ...
        'state.yml',
        '',
        '# Lock directories for checkout-local writes',
        'locks/',
        '',
...
class SetupAutoHandler extends BaseCommand {
  ...
  private async recordRecommendedGrants(cwd: string): Promise<void> {
    const update = async () => {
      ...
      for (const grant of plan.kept) {
        ...
        // branch; bound it here, where this line is built.
        console.log(
          colors.dim(
            `  - Kept ${grant.name}: ${displayPolicyValue(grant.value)} ` +
              '(already answered; change it with `tbd policy`)',
          ),
        );
      }
      if (plan.added.length === 0) {
    ...
    if (this.ctx.dryRun) {
      ...
      await withAgentsMdLock(cwd, update);
    }
  }
  ...
  private async installTierAgentSurface(
    ...
    try {
      ...
      // Only a hard stop aborts the run (see SetupHardStopError). Everything
      // else, including a refused target, is this surface's failure: the rest
      // still install, the summary prints, and the exit code is nonzero.
      if (error instanceof SetupHardStopError) {
        throw error;
      }
  ...
  private async installPortableSurface(cwd: string): Promise<AutoSetupResult> {
    ...
    try {
      ...
      // Before the inspect that reads it and the write that follows, so neither
      // one follows a linked `.agents` out of the project.
      await assertSafeManagedArtifactTarget(portable, { projectRoot: cwd, allowMissing: true });
  ...
  private async installClaudeSurface(cwd: string): Promise<AutoSetupResult> {
    ...
    const claudePaths = getClaudePaths(cwd);
    try {
      // Every file this surface writes, checked before the first read or write.
      // A linked `.claude` used to get the settings, both scripts, the hook
      // script and the skill written through it before anything complained.
      for (const target of claudeSurfaceTargets(cwd)) {

...
This release gives PR review requests one precise meaning and records what agents may do
in a project. Five named requests, from “Review PR #N” to “Make sure PR #N is reviewed
and merged”, run under one review-state contract with a merge gate.
Policy grants in `AGENTS.md` record the user’s standing consent for GitHub actions,
merging, stacked PRs, sub-agents, review requirements, and Linear sync, and one setup
process asks about them.
Delegation to sub-agents follows one procedure with provider-neutral model tiers, and
setup generates tier agent definitions for Claude Code and Codex.
The repository format is unchanged (`f08`); the generated integration format moves to
`f100`.

> [!IMPORTANT]
> Upgrade every clone that runs `tbd setup` in a shared repository before anyone commits
> the regenerated agent files or records grants.
> Setup now stamps `AGENTS.md`, the generated skills, and the tier agent definitions
> with integration format `f100`, split from the repository format.
> tbd 0.9.0 and older refuse to rewrite an `f100` surface: their `tbd setup` stops with
> “generated by a newer tbd” and an upgrade message, and their `tbd doctor` reports the
> files as too new. This is deliberate, because a release without grant support would
> delete the policy block when regenerating `AGENTS.md`. Nothing else changes for
> teammates still on 0.9.0: the repository stays on `f08`, so issue commands, sync, and
> the session hooks keep working until they upgrade.
...
- **PR review lifecycle**: The skill routes five review requests to shortcuts with
  defined end states. “Review PR #N” publishes one senior engineering review at a pinned
  head. “Address the reviews on PR #N” gives every finding a disposition, pushes
  confirmed fixes, gets required CI green, and replies.
  “Review and fix PR #N”, “Get PR #N merge-ready”, and “Make sure PR #N is reviewed and
  merged” run the new `review-and-merge-prs` orchestration in fix, merge-ready, and
  merge modes. One review round is the default, and another runs only after the user
  confirms it. Merging requires the merge gate: review requirements met, every finding
  dispositioned, CI final and green at an unchanged head, and the `github-merge` policy
  permitting the merge, never with `--admin`.
- **Review marker format**: Every published review starts with a hidden, versioned
  marker, `<!-- tbd:review v=1 id=A kind=senior pr=306 round=1 head=… base=… -->`, and a
  visible header recording the reviewer’s requested tier, coverage, and tests run.
  Finding IDs are the review letter plus a number (`A1`), unique within the PR. Every
  finding gets exactly one of four dispositions (fixed, rebutted, declined, or
  deferred), each with required evidence, and a disposition reply carries its own
  `<!-- tbd:dispositions v=1 review=A head=… -->` marker, so “what is still open on this
  PR?” has one answer across sessions and agents.
- **Policy grants and `tbd policy`**: Seven policies (`github-workflows`,
...
- **One setup process**: `tbd shortcut setup-tbd` sets up a new project and reviews the
  ...
  the `gh` authentication, stack tooling, and Linear sync the grants need, and verifies.
  `tbd setup --auto --policies=recommended` records the recommended value for each
  unanswered policy and keeps answered ones.
  Setup output after a fresh setup, a Beads migration, or an upgrade points the agent to
  `setup-tbd` with the number of unanswered policies.
- **Sub-agent delegation and tier agent definitions**:
  `tbd shortcut delegate-to-subagents` is one delegation procedure for any platform:
  check the `subagents` grant, split work by role, assign a tier (strong, moderate, or
  fast, by model rank and reasoning level within the provider), write self-contained
  briefs, and verify every claim.
  Setup now installs six surfaces: the new `claude-agents` and `codex-agents` surfaces
  generate `tbd-strong-max`, `tbd-strong`, `tbd-moderate`, and `tbd-fast` in
  `.claude/agents/` and `.codex/agents/`, with models and reasoning levels from the
  dated suggestions in `agent-model-tiers`. Setup refreshes them on every run; removing
  a file’s `DO NOT EDIT` marker keeps it as your override, and `tbd uninstall --confirm`
  removes the generated ones.
...
- **New shortcuts**: `review-and-merge-prs`, `delegate-to-subagents`, `setup-tbd`, and
  three dedicated review passes built on `review-code`: `review-code-security`,
  `review-code-performance`, and `review-code-correctness`.
- **New guidelines**: `agent-policy-grants`, the single definition of the policies, and
  `agent-model-tiers`, the tier definitions with dated model suggestions.
- **Review shortcuts**: `pr-review-workflows` defines the review-state contract, request
  vocabulary, coverage and rounds, and roles.
  `review-github-pr` reviews at a pinned head after a full discovery sweep, publishes a
  formal review tied to that commit by default, and adds follow-up reviews.
  `address-pr-review` matches reviews by marker and confirms each fix with an automated

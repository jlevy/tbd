...
- [x] **V2-041**: "version-based conflict resolution" but detection is hash-based
  ...
  - **Fix:** Rephrase to "hash-based conflict detection with version/timestamp-based merge ordering"
...
- [x] **V2-017**: Stored JSON normalization (defaults present or omitted) is not defined
  ...
  - **Fix:** Add rule: "All writers MUST serialize fully-normalized entities with defaults applied"
...
- [x] **V2-007**: Merge rules omit BaseEntity fields
  ...
  - **Fix:** Add explicit merge rules for `created_at`, `updated_at`, `created_by`, `closed_at`, `version`
...
- [x] **V2-013**: ID entropy comment + collision math are wrong/inconsistent
  ...
  - **Problem:** Code uses `randomBytes(4)` (32 bits) but slices to 6 hex chars (24 bits)
...
- [x] **V2-015**: Display-prefix compatibility vs import mapping is inconsistent
  ...
  - **Fix:** Clarify that imported IDs get new Ceads IDs, display prefix only affects output
...
- [x] **V2-025**: Notes field exists but CLI lacks explicit support
  ...
  - **Fix:** Add `--notes` and `--notes-file` to update command, show notes in `cead show`
...
- [x] **V2-033**: Design goal says `cead import beads` but CLI spec says `cead import <file>`
...
- [x] **V2-001**: Git write flow can clobber user index / staged changes
  ...
  - **Problem:** Plumbing operations use current repo index, risks destroying staged changes
...
- [x] **V2-002**: Local working copy location for `.ceads-sync/` is undefined
  ...
  - **Resolution:** Added section 2.6 Local Storage Model clarifying cache-based approach
...
- [x] **V2-003**: Missing rule for not leaving untracked `.ceads-sync/` noise on main
  ...
  - **Resolution:** Addressed in Local Storage Model - files live in cache, not working tree

- [?] **V2-004**: `git show ceads-sync:` vs remote tracking branch ambiguity
  - **Location:** §3.3.1 Reading from Sync Branch
  - **Problem:** After fetch, should read from `origin/ceads-sync` not local `ceads-sync`
...
1. **V2-004**: Should we explicitly define when to read from `origin/ceads-sync` vs local `ceads-sync`?

2. **V2-012**: Should we add a note about clock skew or defer HLC to Phase 2?

3. **V2-016**: Is file-per-mapping worth the complexity, or is single mapping file acceptable?

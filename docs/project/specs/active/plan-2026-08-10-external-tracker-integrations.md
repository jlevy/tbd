...
3. **External writes are journaled before they happen** (write-ahead intents), and every
   ...
   removes the journal.
   Replay also revalidates a user-comment intent against the current bead, link, and
   unpushed local entry.
   An unlink or already-recorded entry supersedes the stale intent, which is consumed
...
- **Identity.** An inbound comment’s identity is the provider’s immutable comment UUID.
  ...
  same run cannot re-plan that comment with a fresh UUID. Before replay, the engine
  revalidates that the bead still links the same external item and that the exact local
  entry is still unpushed.
  A missing link/entry or an entry that already carries a provider id means the durable
...
2. **Archive conflict losers, then write intents** for every planned external write,
   ...
   item exists. A live item reconciles normally; before its first bridge record, the
   journaled creation patch is the base, so provider changes made during failed
   follow-up work are not mistaken for local outbound changes.
   A confirmed absence is treated as pending—not orphaned—even in pull-only mode, which
   does not replay provider writes.
...
- **Remote item archived or deleted** → bridge `state: orphaned`, reported by `status`
  ...
  remains. The create patch supplies the first base when no bridge record exists, so an
  intervening provider edit pulls or conflicts instead of being overwritten.
  If absent, the link is omitted from orphan output until create replay succeeds or
  unlink cancels it.
...
- **Crash replay**: `intents.test.ts` simulates a crash after every step of the apply
  ...
  A provider-accepted create whose attachment fails before the first bridge record then
  receives a remote edit; pull-only uses the journaled creation snapshot to take that
  edit without provider I/O, and the following full run completes and consumes the
  original journal without reverting it.

...
- Validate-all-then-render: resolve every ID first; any unknown aborts with the full
  ...
  subset and reports skips on stderr; exit non-zero if anything was skipped.
...
replace the `| jq` spec-table recipe with `list --specs`/`--json`.

## Implementation Plan

### Phase 1: Kill the observed loops (bulk show, variadic doc readers, docs)

- [ ] `show <ids...>`: validation contract, dedupe, delimiter, per-issue `--max-lines`,
  bulk parent suppression, `--json` array for 2+ IDs.
  (tbd-r2zr, elevated to P1.)
- [ ] `DocCommandHandler` variadic queries for `guidelines`/`shortcut`/`template` +
  `docs show <names...>`; fail-closed resolution; preamble-once output.
- [ ] Point-of-need doc pass: skill-baseline/brief/minimal tables, `tbd-prime.md`,
...
- [ ] `dep add`/`dep remove` variadic `<depends-on...>`.
- [ ] `create --depends-on <id>` (repeatable); update `plan-implementation-with-beads`
  to use it.
- [ ] `create`/`update` `--spec` accepts unique basename/suffix (same matcher as
...
- [ ] Did-you-mean on unknown IDs; ID-matching in `search`; overflow-hint map for
  remaining single-target commands.
- [ ] Purge bash recipes: `tbd-docs.md` grep recipe, `update-specs-status.md` jq
  pipeline.
...
- **tryscript goldens** (`.tryscript.md` harness), mirroring the bulk-mutator goldens:
  ...
  `--ignore-missing` + non-zero exit, `--json` array vs single-ID object, per-issue
  `--max-lines`, parent suppression in bulk vs auto-display single); variadic
  `guidelines`/`template` (two names, order, one bad name fails closed); `dep add` with
  3 blockers then `dep list`; `create --depends-on` twice then `blocked`; unknown-ID
  did-you-mean output; `search` by partial display ID; overflow hint for `create`.
...
- **`guidelines --category <cat>` printing content** (group loader) instead of being
  ...
  second way to do it?
- **Delimiter format for bulk show:** plain dim rule vs `--- id: … ---` YAML-ish header.
  Decide at golden time; only constraint is stable and grep-able.

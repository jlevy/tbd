---
close_reason: Integrated split local/remote sync refs across sync, doctor, status, init, setup, and uninstall commands including diagnostics and managed-branch cleanup.
closed_at: 2026-02-26T06:38:49.050Z
created_at: 2026-02-25T20:10:35.054Z
dependencies: []
id: is-01kjb6sfnf7ww2ezegg3fa4nr8
kind: task
labels: []
parent_id: is-01kjb6s8sc4b9cveaamzeasdxa
priority: 1
spec_path: docs/project/specs/active/plan-2026-02-25-multi-worktree-sync-branch-isolation.md
status: closed
title: Integrate split local/remote sync branch semantics in sync, status, and doctor
type: is
updated_at: 2026-03-09T02:47:25.394Z
version: 7
---
Use local sync branch for local operations while fetching/merging/pushing against canonical remote sync branch, plus stale branch cleanup and diagnostics.
